...
def b_dice_loss(pred, target, alpha, smooth=1e-6):
    ...
    # Combined loss
    loss = alpha * b_loss + (1 - alpha)*(1 - mean_dice)
    return loss, b_loss, 1-mean_dice
...
def train(rank, world_size, model, train_loader, optimizer, epoch, batch_size, alpha):
    model.train()
    ...
    for images, masks in tqdm(train_loader, desc=f"Epoch {epoch + 1} Training", disable=(rank != 0)):
        ...
        loss, l_hd, l_dsc = b_dice_loss(outputs, masks, alpha)
    ...
    if rank == 0:
        print(f"Epoch {epoch + 1} - Loss: {avg_loss:.4f} - L_B: {avg_hd:.4f} - L_DSC: {avg_dsc:.4f} - Alpha: {alpha:.2f} - Dice: {avg_dice:.2f}, Prec: {avg_prec:.2f}, Rec: {avg_rec:.2f}, HD95: {avg_hd95:.2f}, ASSD: {avg_assd:.2f}, IPE: {avg_ipe:.2f}, Mem: {avg_mem:.2f}")
...
def validate(rank, world_size, model, val_loader, batch_size, alpha):
    model.eval()
    ...
    if dist.get_rank() == 0:
        print(f"Val. Loss: {avg_loss:.4f}, L_B: {avg_hd:.4f}, L_DSC: {avg_dsc:.4f}, Val. DSC: {avg_dice:.2f}, Val. Precision: {avg_prec:.2f}, Val. Recall: {avg_rec:.2f}, Val. HD95: {avg_hd95:.2f}, Val. ASSD: {avg_assd:.2f}, Val. IPE: {avg_ipe:.2f}, Val. Mem: {avg_mem:.2f}")
        
    return avg_loss, avg_hd, avg_dsc, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem

# Testing function (DDP-compatible)
def test(rank, world_size, model, test_loader, batch_size, alpha):
    model.eval()
    ...
    if dist.get_rank() == 0:
        print(
            f"Te. Loss: {avg_loss:.4f} | L_B: {avg_hd:.4f} | L_DSC: {avg_dsc:.4f} | Alpha: {alpha:.2f} | DSC: {avg_dice:.2f}% | "
...
def write_test_result(test_log_path, test_loss, test_hd, test_dsc, best_lambda, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops):
    if dist.get_rank() == 0:
        with open(test_log_path, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['test_loss', 'test_b', 'test_dsc', 'best_alpha', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'])
...
def main():
    ...
    if rank == 0:
        ...
        init_log_csv(train_log_path, [
            'epoch', 'train_loss', 'train_b', 'train_dsc', 'train_alpha', 'train_dice', 'train_precision', 'train_recall',
            'train_hd95', 'train_assd', 'train_ipe', 'train_mem', 'val_loss', 'val_b', 'val_dsc', 'val_dice', 'val_precision',
            'val_recall', 'val_hd95', 'val_assd', 'val_ipe', 'val_mem'
        ])
        
        test_log_path = f'logs/test_log_PT_{lf_name}.csv'
        init_log_csv(test_log_path, [
            'test_loss', 'test_b', 'test_dsc', 'best_alpha', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'
            ])
    ...
    if rank == 0:
        append_log_csv(test_log_path, [test_loss, test_hd, test_dsc, early_stopping.best_alpha, test_dice, test_precision, test_recall,
