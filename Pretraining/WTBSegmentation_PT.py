...
class EarlyStopping:
    ...
    def __call__(self, model, current_score, epoch, lf_name):
...
def compute_distance_map_flipped(mask, iterations=10):
    inv = 1 - mask
    dist = torch.zeros_like(mask)
    x = inv.clone()
    ...
    max_val = dist.max().clamp(min=1.0)
    phi = max_val - dist  # large near boundary, small far away
    return phi

def distance_map_loss(pred, target, reduction='mean'):
    # CE term (or Dice, focal, etc.)
    ce = F.binary_cross_entropy(pred, target, reduction='none')  # (B,1,H,W)
    phi = compute_distance_map_flipped(target)                   # (B,1,H,W)
    loss = (1.0 + phi) * ce
...
def hausdorff_loss(pred, target, reduction='mean'):
    ...
    for p, g in zip(pred, target):
        ...
        d_gt = compute_distance_map(g.squeeze(0))
        d_pred = compute_distance_map((p > 0.5).float().squeeze(0))  # binarized pred for distance
...
def train(rank, world_size, model, train_loader, optimizer, epoch, batch_size):
    model.train()
    ...
    for images, masks in tqdm(train_loader, desc=f"Epoch {epoch + 1} Training", disable=(rank != 0)):
        ...
        loss = distance_map_loss(outputs, masks)
    ...
    avg_mem_gb = total_mem / total_batches if total_batches > 0 else 0.0
    ...
    metrics /= world_size
    avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem = metrics.tolist()

    if rank == 0:
        print(f"Epoch {epoch + 1} - Loss {avg_loss:.4f}, Dice {avg_dice:.2f}, Prec {avg_prec:.2f}, Rec {avg_rec:.2f}, HD95 {avg_hd95:.2f}, ASSD {avg_assd:.2f}, IPE {avg_ipe:.2f}, Mem {avg_mem:.2f}")
    return avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem

# Validation loop with Dice Score reporting (DDP-compatible)
def validate(rank, world_size, model, val_loader, batch_size):
    model.eval()
    ...
    if dist.get_rank() == 0:
        print(f"Val. Loss: {avg_loss:.4f}, Val. DSC: {avg_dice:.2f}, Val. Precision: {avg_prec:.2f}, Val. Recall: {avg_rec:.2f}, Val. HD95: {avg_hd95:.2f}, Val. ASSD: {avg_assd:.2f}, Val. IPE: {avg_ipe:.2f}, Val. Mem: {avg_mem:.2f}")
        
    return avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem

# Testing function (DDP-compatible)
def test(rank, world_size, model, test_loader, batch_size):
    model.eval()
    ...
    if dist.get_rank() == 0:
        print(
            f"Te. Loss: {avg_loss:.4f} | DSC: {avg_dice:.2f}% | "
    ...
    if rank == 0:
        return (
            avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd,
...
# Visualize samples (only on rank 0)
def visualize_random_samples(model, test_loader, rank, lf_name, num_samples=5):
    if dist.get_rank() != 0:
        return  # Only rank 0 should do visualization
    ...
    for i, idx in enumerate(indices):
        image, target_mask = test_loader.dataset[idx]
        image = image.unsqueeze(0).to(rank)
        target_mask = target_mask.unsqueeze(0).to(rank)
...
def write_test_result(test_log_path, test_loss, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops):
    if dist.get_rank() == 0:
        with open(test_log_path, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['test_loss', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'])
            writer.writerow([test_loss, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops])
...
def main():
    ...
    if rank == 0:
        ...
        init_log_csv(train_log_path, [
            'epoch', 'train_loss', 'train_dice', 'train_precision', 'train_recall',
            'train_hd95', 'train_assd', 'train_ipe', 'train_mem', 'val_loss', 'val_dice', 'val_precision',
            'val_recall', 'val_hd95', 'val_assd', 'val_ipe', 'val_mem'
        ])
        
        test_log_path = f'logs/test_log_PT_{lf_name}.csv'
        init_log_csv(test_log_path, [
            'test_loss', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'
            ])
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem = train(
            rank, world_size, model, train_loader, optimizer, epoch, batch_size
        )
        val_loss, val_dice, val_precision, val_recall, val_hd95, val_assd, val_ipe, val_mem = validate(
            rank, world_size, model, val_loader, batch_size
        )
        scheduler.step(val_loss)

        if rank == 0:
            append_log_csv(train_log_path, [
                epoch + 1, train_loss, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem,
                val_loss, val_dice, val_precision, val_recall, val_hd95, val_assd, val_ipe, val_mem
            ])
        ...
        early_stopping(model, val_loss, epoch+1, lf_name)
    ...
    test_loss, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops = test(
        rank, world_size, best_model.module, test_loader_c, batch_size
    )
    
    if rank == 0:
        append_log_csv(test_log_path, [test_loss, test_dice, test_precision, test_recall,
                          test_hd95, test_assd, test_ipe, test_mem, test_flops])
    
    # print('Testing on hard samples')
    # test_loss, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops = test(
    #     rank, world_size, best_model.module, test_loader_u, batch_size
    # )
    # if rank == 0:
    #     append_log_csv(test_log_path, [test_loss, test_dice, test_precision, test_recall,
    #                       test_hd95, test_assd, test_ipe, test_mem, test_flops])
