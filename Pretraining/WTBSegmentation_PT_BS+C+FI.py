...
def bs_cloc_loss(pred, target, beta, smooth=1e-6):
    ...
    # Combined loss
    loss = bs_l + beta * (c_l + fi_l) + bce_loss
    return loss, bs_l, c_l, fi_l
...
def train(rank, world_size, model, train_loader, optimizer, epoch, batch_size, beta):
    ...
    running_hd = running_dsc = running_fi = 0.0
    total_batches = 0
    ...
    for images, masks in tqdm(train_loader, desc=f"Epoch {epoch + 1} Training", disable=(rank != 0)):
        ...
        # outputs = outputs[:,0:1,:,:]
        # masks = masks[:,0:1,:,:]
        loss, l_hd, l_dsc, l_fi = bs_cloc_loss(outputs, masks, beta)
    ...
    metrics /= world_size
    avg_hd, avg_dsc, avg_fi, avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem = metrics.tolist()

    if rank == 0:
        print(f"Epoch {epoch + 1} - Loss: {avg_loss:.4f} - L_BS: {avg_hd:.4f} - C_Loc: {avg_dsc:.4f} - L_Fi: {avg_fi:.4f} - Beta: {beta:.2f} - Dice: {avg_dice:.2f}, Prec: {avg_prec:.2f}, Rec: {avg_rec:.2f}, HD95: {avg_hd95:.2f}, ASSD: {avg_assd:.2f}, IPE: {avg_ipe:.2f}, Mem: {avg_mem:.2f}")
    return avg_loss, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem
...
def validate(rank, world_size, model, val_loader, batch_size, beta):
    ...
    avg_loss, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem = metrics.tolist()

    if dist.get_rank() == 0:
        print(f"Val. Loss: {avg_loss:.4f}, L_BS: {avg_hd:.4f}, C_Loc: {avg_dsc:.4f}, L_Fi: {avg_fi:.4f}, Val. DSC: {avg_dice:.2f}, Val. Precision: {avg_prec:.2f}, Val. Recall: {avg_rec:.2f}, Val. HD95: {avg_hd95:.2f}, Val. ASSD: {avg_assd:.2f}, Val. IPE: {avg_ipe:.2f}, Val. Mem: {avg_mem:.2f}")
...
def test(rank, world_size, model, test_loader, batch_size, beta):
    ...
    if dist.get_rank() == 0:
        print(
            f"Te. Loss: {avg_loss:.4f} | L_BS: {avg_hd:.4f} | C_Loc: {avg_dsc:.4f} | L_Fi: {avg_fi:.4f} | Beta: {beta:.2f} | DSC: {avg_dice:.2f}% | "
    ...
    if rank == 0:
        return (
            avg_loss, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd,
...
def main():
    ...
    if rank == 0:
        ...
        init_log_csv(train_log_path, [
            'epoch', 'train_loss', 'train_bs', 'train_cloc', 'train_fi', 'train_beta', 'train_dice', 'train_precision', 'train_recall',
        ...
        init_log_csv(test_log_path, [
            'test_loss', 'test_bs', 'test_cloc', 'test_fi', 'best_beta', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'
        ])
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_hd, train_dsc, train_fi, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem = train(
            rank, world_size, model, train_loader, optimizer, epoch, batch_size, alpha
        )
        
        val_loss, val_hd, val_dsc, val_fi, val_dice, val_precision, val_recall, val_hd95, val_assd, val_ipe, val_mem = validate(
        ...
        if rank == 0:
            append_log_csv(train_log_path, [
                epoch + 1, train_loss, train_hd, train_dsc, train_fi, alpha, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem,
    ...
    test_loss, test_hd, test_dsc, test_fi, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops = test(
        rank, world_size, best_model.module, test_loader_c, batch_size, early_stopping.best_alpha
    )
    
    if rank == 0:
        append_log_csv(test_log_path, [test_loss, test_hd, test_dsc, test_fi, early_stopping.best_alpha, test_dice, test_precision, test_recall,
