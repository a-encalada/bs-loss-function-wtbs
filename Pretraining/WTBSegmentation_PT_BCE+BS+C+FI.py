...
def compute_distance_map(mask, iterations=10):
    """
    Approximates distance transform INSIDE the FG region.
    mask: (B,1,H,W) binary {0,1}
    Returns: distance where center of FG = large, border = 0, BG = 0
    """
    mask = mask.float()
    dist = torch.zeros_like(mask)

    # current foreground region that survives erosion
    current = mask.clone()

    for step in range(1, iterations + 1):
        # Erode: only pixels whose 3x3 neighborhood is all 1 survive
        eroded = (F.avg_pool2d(current, kernel_size=3, stride=1, padding=1) == 1.0).float()

        # ring = pixels removed in this step (they were inside FG before, now gone)
        ring = current - eroded  # still inside original FG

        # assign distance = step to these pixels
        dist += step * ring
    ...
    # border pixels were never “removed” → distance 0 by design
    # background always 0
    return dist

def _dilate(x, k=2):
    # x: (B,1,H,W)
    # pad right and bottom by 1 to keep same size after 2x2 pool
    x_pad = F.pad(x, (0,1,0,1), mode='replicate')  # (left,right,top,bottom)
    y = F.max_pool2d(x_pad, kernel_size=2, stride=1)  # -> (B,1,H,W)
    return y

def _erode(x, k=2):
    x_inv = 1.0 - x
    x_inv_pad = F.pad(x_inv, (0,1,0,1), mode='replicate')
    y_inv = F.max_pool2d(x_inv_pad, kernel_size=2, stride=1)
    return 1.0 - y_inv

def bs_loss(pred, target, alpha=0.8, threshold=0.5, eps=1e-6, kernel_size=2):
    """
    Boundary-sensitive BS-loss from the paper.

    Args:
        pred:   (B,1,H,W) prediction probabilities in [0,1]
        target: (B,1,H,W) ground-truth mask in {0,1}
        alpha:  weight for boundary false pixels (0.6–0.9 in paper)
    ...
    # 2) Edge expansion & shrinking maps
    #    EM = dilated mask; outer ring used as (EM - mask)
    #    SM = eroded mask; inner ring used as (mask - SM)
    EM_P  = _dilate(P,  kernel_size)
    SM_P  = _erode(P,   kernel_size)
    EM_GT = _dilate(GT, kernel_size)
    SM_GT = _erode(GT,  kernel_size)

    # Bands
    inner_P  = P  * (1.0 - SM_P)           # P - SM^P
    outer_P  = (EM_P  - P).clamp(0, 1)     # EM^P - P

    inner_GT = GT * (1.0 - SM_GT)          # GT - SM^GT
    outer_GT = (EM_GT - GT).clamp(0, 1)    # EM^GT - GT

    # 3) Basic regions
    TIG          = P * GT                  # true intersection
    P_minus_TIG  = P  * (1.0 - GT)         # prediction-only pixels
    GT_minus_TIG = GT * (1.0 - P)          # GT-only pixels

    # 4) False pixels near boundaries
    FBP_in  = P_minus_TIG  * inner_P       # (P - TIG) ∩ (P - SM^P)
    FBP_out = GT_minus_TIG * outer_P       # (GT - TIG) ∩ (EM^P - P)

    FBG_in  = GT_minus_TIG * inner_GT      # (GT - TIG) ∩ (GT - SM^GT)
    FBG_out = P_minus_TIG  * outer_GT      # (P - TIG) ∩ (EM^GT - GT)

    # 5) False pixels far from boundaries
    FIP = (P_minus_TIG  - FBP_in  - FBG_out).clamp(min=0.0)
    FIG = (GT_minus_TIG - FBP_out - FBG_in).clamp(min=0.0)

    # 6) Weighted false pixels (Equations 7–8)
    WF_P  = alpha * (FBP_in  + FBG_out) + (1.0 - alpha) * FIP
    WF_GT = alpha * (FBP_out + FBG_in)  + (1.0 - alpha) * FIG

    # 7) Sum over pixels
    #    shape: (B,)
    TIG_sum  = TIG.view(TIG.size(0), -1).sum(dim=1)
    WF_P_sum = WF_P.view(WF_P.size(0), -1).sum(dim=1)
    WF_GT_sum= WF_GT.view(WF_GT.size(0), -1).sum(dim=1)

    # 8) BS-loss per sample (Dice-style)
    numerator   = 2.0 * TIG_sum
    denominator = 2.0 * TIG_sum + WF_P_sum + WF_GT_sum + eps

    loss_per_sample = 1.0 - numerator / denominator
    return loss_per_sample.mean()

def c_loc(pred, target, reduction="mean", eps=1e-6):
    """
    Geometric C_loc as in the paper.
    pred, target must be B×1×H×W *binary masks* (0/1)
    Returns unnormalized C_loc in pixel units.
    """
    B, _, H, W = pred.shape

    # convert probabilities to binary (paper uses binary)
    pred_bin = (pred>0.5).float()
    target = target.float()

    # column projections: shape (B, W)
    pred_col = pred_bin.sum(dim=2).squeeze(1)
    tgt_col  = target.sum(dim=2).squeeze(1)

    # row projections: shape (B, H)
    pred_row = pred_bin.sum(dim=3).squeeze(1)
    tgt_row  = target.sum(dim=3).squeeze(1)

    # L1 differences (SUM, not mean)
    col_loss = torch.abs(pred_col - tgt_col).sum(dim=1)   # per image
    row_loss = torch.abs(pred_row - tgt_row).sum(dim=1)   # per image

    loss = col_loss + row_loss  # geometric, up to O(H+W)
...
def _soft_dilate(x, k):
    # x: (B,1,H,W)
    pad = k // 2
    return F.max_pool2d(x, kernel_size=k, stride=1, padding=pad)

def _soft_erode(x, k):
    # min-pool via max-pool on negated tensor
    pad = k // 2
    return -F.max_pool2d(-x, kernel_size=k, stride=1, padding=pad)

def soft_closing(x, k):
    return _soft_erode(_soft_dilate(x, k), k)

def fg_int_loss(pred, target, k=5, eps=1e-6):
    p = (pred>0.5).float().clamp(0, 1)
    g = target.float()

    c = soft_closing(p, k)
    hole_score = F.relu(c - p) * g

    per_img_holes = hole_score.sum(dim=(1,2,3))
    #per_img_area  = g.sum(dim=(1,2,3))

    return per_img_holes.mean()

def bs_cloc_loss(pred, target, beta, smooth=1e-6):
    # plt.imshow(target[0,:,:,:].squeeze().cpu().numpy(), cmap='gray')
    # plt.imshow(pred[0,:,:,:].squeeze().cpu().numpy(), cmap='gray')
    # plt.savefig(f"holaaa.png")
    # plt.show()
    
    # Boundary loss
    bs_l = bs_loss(pred, target)
    c_l = c_loc(pred, target)
    fi_l = fg_int_loss(pred, target=target)
    bce_l = F.binary_cross_entropy(pred, target, reduction='mean')
    
    # Combined loss
    loss = bce_l + bs_l + beta * (c_l + fi_l)
    return loss, bce_l, bs_l, c_l, fi_l
...
def train(rank, world_size, model, train_loader, optimizer, epoch, batch_size, beta):
    ...
    running_hd = running_dsc = running_fi = running_bce = 0.0
    total_batches = 0
    ...
    for images, masks in tqdm(train_loader, desc=f"Epoch {epoch + 1} Training", disable=(rank != 0)):
        ...
        # outputs = outputs[:,0:1,:,:]
        # masks = masks[:,0:1,:,:]
        loss, bce_l, l_hd, l_dsc, l_fi = bs_cloc_loss(outputs, masks, beta)
    ...
    metrics = torch.tensor([
        running_bce / len(train_loader),
        running_hd / len(train_loader),
        running_dsc / len(train_loader),
        running_fi / len(train_loader),
    ...
    metrics /= world_size
    avg_bce, avg_hd, avg_dsc, avg_fi, avg_loss, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem = metrics.tolist()

    if rank == 0:
        print(f"Epoch {epoch + 1} - Loss: {avg_loss:.4f} - L_BCE: {avg_bce:.4f} - L_BS: {avg_hd:.4f} - C_Loc: {avg_dsc:.4f} - L_Fi: {avg_fi:.4f} - Beta: {beta:.2f} - Dice: {avg_dice:.2f}, Prec: {avg_prec:.2f}, Rec: {avg_rec:.2f}, HD95: {avg_hd95:.2f}, ASSD: {avg_assd:.2f}, IPE: {avg_ipe:.2f}, Mem: {avg_mem:.2f}")
    return avg_loss, avg_bce, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem
...
def validate(rank, world_size, model, val_loader, batch_size, beta):
    ...
    with torch.no_grad():
        for images, masks in tqdm(val_loader, desc="Validation", disable=(dist.get_rank() != 0)):
            ...
            t0 = time.time()
            outputs = model(images)
            # plt.imshow(outputs[0,:,:,:].squeeze().cpu().numpy(), cmap='gray')
            # plt.show()
            torch.cuda.synchronize()
            infer_time = time.time() - t0
    ...
    metrics = torch.tensor([
        ...
        running_bce / len(val_loader),
        running_hd / len(val_loader),
        running_dsc / len(val_loader),
        running_fi / len(val_loader),
    ...
    avg_loss, avg_bce, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd, avg_ipe, avg_mem = metrics.tolist()

    if dist.get_rank() == 0:
        print(f"Val. Loss: {avg_loss:.4f}, L_BCE: {avg_bce:.4f}, L_BS: {avg_hd:.4f}, C_Loc: {avg_dsc:.4f}, L_Fi: {avg_fi:.4f}, Val. DSC: {avg_dice:.2f}, Val. Precision: {avg_prec:.2f}, Val. Recall: {avg_rec:.2f}, Val. HD95: {avg_hd95:.2f}, Val. ASSD: {avg_assd:.2f}, Val. IPE: {avg_ipe:.2f}, Val. Mem: {avg_mem:.2f}")
...
def test(rank, world_size, model, test_loader, batch_size, beta):
    ...
    metrics = torch.tensor([
        ...
        running_bce / len(test_loader),
        running_hd / len(test_loader),
        running_dsc / len(test_loader),
        running_fi / len(test_loader),
    ...
    if dist.get_rank() == 0:
        print(
            f"Te. Loss: {avg_loss:.4f} | L_BCE: {avg_bce:.4f} | L_BS: {avg_hd:.4f} | C_Loc: {avg_dsc:.4f} | L_Fi: {avg_fi:.4f} | Beta: {beta:.2f} | DSC: {avg_dice:.2f}% | "
    ...
    if rank == 0:
        return (
            avg_loss, avg_bce, avg_hd, avg_dsc, avg_fi, avg_dice, avg_prec, avg_rec, avg_hd95, avg_assd,
...
def write_test_result(test_log_path, test_loss, test_hd, test_dsc, test_fi, best_lambda, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops):
    if dist.get_rank() == 0:
        with open(test_log_path, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['test_loss', 'test_bs', 'test_cloc', 'test_fi', 'best_beta', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'])
            writer.writerow([test_loss, test_hd, test_dsc, test_fi, best_lambda, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops])
...
def main():
    ...
    if rank == 0:
        ...
        init_log_csv(train_log_path, [
            'epoch', 'train_loss', 'train_bce', 'train_bs', 'train_cloc', 'train_fi', 'train_beta', 'train_dice', 'train_precision', 'train_recall',
            'train_hd95', 'train_assd', 'train_ipe', 'train_mem', 'val_loss', 'val_bs', 'val_cloc', 'val_fi', 'val_dice', 'val_precision',
            'val_recall', 'val_hd95', 'val_assd', 'val_ipe', 'val_mem'
        ])
        test_log_path = f'logs/test_log_PT_{lf_name}.csv'
        init_log_csv(test_log_path, [
            'test_loss', 'test_bce', 'test_bs', 'test_cloc', 'test_fi', 'best_beta', 'test_dice', 'test_precision', 'test_recall', 'test_hd95', 'test_assd', 'test_ipe', 'test_mem', 'test_flops'
        ])
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_bce, train_hd, train_dsc, train_fi, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem = train(
            rank, world_size, model, train_loader, optimizer, epoch, batch_size, alpha
        )
        
        val_loss, val_bce, val_hd, val_dsc, val_fi, val_dice, val_precision, val_recall, val_hd95, val_assd, val_ipe, val_mem = validate(
        ...
        if rank == 0:
            append_log_csv(train_log_path, [
                epoch + 1, train_loss, train_bce, train_hd, train_dsc, train_fi, alpha, train_dice, train_precision, train_recall, train_hd95, train_assd, train_ipe, train_mem,
                val_loss, val_hd, val_dsc, val_fi, val_dice, val_precision, val_recall, val_hd95, val_assd, val_ipe, val_mem
            ])
    ...
    test_loss, test_bce, test_hd, test_dsc, test_fi, test_dice, test_precision, test_recall, test_hd95, test_assd, test_ipe, test_mem, test_flops = test(
        rank, world_size, best_model.module, test_loader_c, batch_size, early_stopping.best_alpha
    )
    
    if rank == 0:
        append_log_csv(test_log_path, [test_loss, test_bce, test_hd, test_dsc, test_fi, early_stopping.best_alpha, test_dice, test_precision, test_recall,
